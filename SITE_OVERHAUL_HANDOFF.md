...
**The posters exist now** (`public/img/loom/alphabet-blanket.webp` and
`name-bib.webp`, committed), because the generator was fixed and made
hermetic; it no longer needs a machine with webfont access, so this is no
longer an owner-only task. Then PR 8 (the product page).
