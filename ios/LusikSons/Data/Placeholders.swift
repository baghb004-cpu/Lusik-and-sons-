...
/// A coming-soon catalog item — the Swift mirror of a web CATALOG entry
/// with `status: "placeholder"`. Not a `Product`: it has no checkout key,
/// no price, no photos yet. Its `key` IS load-bearing — it's the
/// product_key the waitlist Function stores, and it must match the web
/// catalog so the admin Notify sweep sees app + website signups as one
/// list per product.
struct PlaceholderProduct: Identifiable, Hashable {
    let key: String
    let name: String
    let tagline: String
...
/// The four placeholders (web catalog.js parity, copy verbatim). When
/// Lusik flips one live on the website, move it into Catalog.products
/// with its trusted checkout key + photos and delete it here.
enum Placeholders {
    static func inCategory(_ slug: String) -> [PlaceholderProduct] {
        all.filter { $0.categorySlug == slug }
    }

    static let all: [PlaceholderProduct] = [
        PlaceholderProduct(
            key: "towel-hand",
