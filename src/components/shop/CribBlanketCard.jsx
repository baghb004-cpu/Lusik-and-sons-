...
export function CribBlanketCard({ product, spec, trail, onAddCustom, onBuyNow, onCartFeedback, soldOut = false, notifyKey, immersive = false }) {
  ...
  return (
    // Immersive (mobile sheet): drop outer chrome; photos move to the
    // full-screen backdrop, so the gallery runs photosHidden (Apple color row
    // stays live to drive the body-color choice).
    <div className={immersive ? "fade-in" : "fade-in max-w-6xl mx-auto px-6 lg:px-12 py-8 lg:py-12"}>
      ...
      {/* Mobile buy sheet — persistent on mobile. Suppressed in immersive
          mode (the immersive sheet is the buy surface there). */}
      {!immersive && (
        <MobilePurchaseBar
          visible={!soldOut}
