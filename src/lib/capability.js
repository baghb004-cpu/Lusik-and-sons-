...
import { resolveTier, normalizeTier, TIER_STORAGE_KEY, TIER_SESSION_KEY, } from "./capabilityTier.js";
export const CAPABILITY_EVENT = "capability:change";
const RANK = { full: 0, lean: 1, core: 2 };
...
let state = { tier: "full", reasons: ["ssr"], override: null, signals: {} };
let initialized = false;
let urlConsumed = false;
let memChoice = null; // the visitor's choice when storage is blocked
let changeTimer;
export function getTier() { return state.tier; }
export function getCapability() { return { ...state, reasons: state.reasons.slice(), signals: state.signals }; }
/** What detection alone would pick right now (ignores every override). */
export function getDetectedTier() { return resolveTier(state.signals, null).tier; }
/** The visitor's own persisted choice (footer toggle), if any. */
export function getTierChoice() {
    if (memChoice)
        return memChoice;
    try {
        return normalizeTier(localStorage.getItem(TIER_STORAGE_KEY));
    }
    catch {
        return null;
    }
 ...
 */
export function getGpuSignal() {
    if (!state.signals.gpu)
        state.signals.gpu = probeGpu();
...
export function initCapability() {
    ...
    initialized = true;
    if (!ENABLED) {
        apply({ tier: "full", reasons: ["disabled"], override: null, signals: {} });
        return;
    }
...
function readOverride() {
    ...
    try {
        if (!urlConsumed) {
            ...
            const fromUrl = normalizeTier(new URLSearchParams(window.location.search).get(param));
            if (fromUrl)
                sessionStorage.setItem(TIER_SESSION_KEY, fromUrl);
        }
        return normalizeTier(sessionStorage.getItem(TIER_SESSION_KEY));
    }
    catch {
        return null;
    }
...
function watchFirstImage() {
    try {
        const isPhoto = (e) => (e.initiatorType === "img" || e.initiatorType === "link") && /\/img\/|%2Fimg%2F/i.test(e.name);
        const take = (e) => {
            state.signals.firstImageMs = Math.round((e.responseEnd || e.duration) - (e.startTime || 0));
            recompute({ allowRise: false });
        };
        const already = performance.getEntriesByType("resource").find(isPhoto);
        if (already) {
            take(already);
            return;
        }
        const po = new PerformanceObserver((list) => {
            const hit = list.getEntries().find(isPhoto);
            if (hit) {
                take(hit);
                po.disconnect();
            }
...
function bindListeners() {
    ...
    try {
        window.matchMedia("(prefers-reduced-data: reduce)").addEventListener?.("change", onNetworkChange);
    }
    catch { /* unsupported */ }
}
function recompute({ allowRise }) {
    if (!ENABLED)
        return;
...
function apply(next) {
    ...
    if (changed || document.documentElement.dataset.tier !== next.tier) {
        ...
        if (next.override)
            root.dataset.tierOverride = next.override;
        else
            delete root.dataset.tierOverride;
    }
    if (changed)
        window.dispatchEvent(new CustomEvent(CAPABILITY_EVENT, { detail: getCapability() }));
}
