 ...
 * @param {number} input.charW  one character's chart width
 ...
 */
export function layoutText({ text, slotW, charW, gap = 1, align = "center" }) {
  const chars = [...String(text ?? "")];
  if (chars.length === 0 || slotW <= 0 || charW <= 0) {
    return { placements: [], dropped: 0, usedW: 0 };
  }
  // How many characters actually fit: n chars need n*charW + (n-1)*gap.
  let fit = Math.floor((slotW + gap) / (charW + gap));
  if (fit < 0) fit = 0;
  const kept = chars.slice(0, fit);
  const dropped = chars.length - kept.length;

  const usedW = kept.length === 0 ? 0 : kept.length * charW + (kept.length - 1) * gap;
  const startX = align === "center" ? Math.floor((slotW - usedW) / 2) : 0;

  const placements = kept.map((char, i) => ({
    char,
    originX: startX + i * (charW + gap),
  }));
...
export function planLine({ text, slot, chartFor, color, gap = 1, align = "center" }) {
  ...
  // Vertically centre the line in the slot.
  const originY = slot.y + Math.floor((slot.h - charH) / 2);
